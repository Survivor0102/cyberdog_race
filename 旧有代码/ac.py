...
def process_arrow_logic(msg, vision_utils,node_instance):
    """箭头检测具体逻辑"""
    img = vision_utils.msg_to_cv(msg)
    if img is None: return

    # 模拟复杂的箭头检测
    # direction, ... = detect_arrow(img)
    
    # 仅做演示：画个圈表示正在处理
    cv.circle(img, (img.shape[1]//2, img.shape[0]//2), 50, (0, 255, 0), 2)
    cv.putText(img, "Searching Arrow...", (10, 30), cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
    
    vision_utils.show("Arrow_Active", img)


def process_QR_logic(msg, vision_utils,node_instance):
    img = vision_utils.msg_to_cv(msg)
    if img is None: 
        return
    ...
    if wechat_available:
        # --- 核心修改：使用 WeChat QRCode ---
        # detectAndDecode 返回: (data_list, points_list)
        res, points, _ = qr_detector.detectAndDecode(cv_image)
        ...
        if res:
            for i, data in enumerate(res):
                ...
                # 画框
                cv.polylines(cv_image, [pts], True, (0, 255, 0), 2)
                
                # 画文字
                cv.putText(cv_image, data, (pts[0][0], pts[0][1] - 10),
                           cv.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                
                print(f"识别到二维码 (WeChat): {data}")
                cv.imshow("QR_detect", cv_image)
                cv.waitKey(1)
...
class ArrowDetectorNode(DynamicVisionNode):
    def __init__(self):
        super().__init__('arrow_detector', '/rgb_camera/image_raw', process_arrow_logic)
        self.arrow_direction = None

class QRDetectorNode(DynamicVisionNode):
    def __init__(self):
        super().__init__('QR_detector', '/rgb_camera/image_raw', process_QR_logic)
        self.QR_img = []
        self.QR_result = []
...
# zuo 0
    ...
    try:
        ...
        # 停止时的速度
                ...
                #                                 # 建议从 1.0 开始试，如果太慢加大，如果震荡减小。
                #         MAX_VEL = 0.32           # 最大角速度：限制远距离时的最高转速 (原固定值为 0.1)
                #         MIN_VEL_THRESHOLD = 0.15 # 死区阈值：当误差小于此值时，认为已对准
                #         STOP_VEL = 0.0 
                #         current_phase = TEST_DIR 
                ...
                if current_phase == APHASE_SHIBAN:
                    ...
                    WALK_SPEED = 0.15           # 速度 m/s (慢走模式推荐 0.1 ~ 0.3)
